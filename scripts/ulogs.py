...
def main(argv):
  ...
  if len(argv)  == 5:
    ...
    dec1 = set(dec1, -20.)
    dec2 = set(dec2, 30.)
    unique = False if unique != 'y' else True
  ...
  print '<tr><td colspan="4"> <input type="checkbox" name="unique" value="Unique nights only" ',
  ...
  if len(ras[ok]):
    ...
    if unique:
      ...
      for ra,dec,run,night,number,target,iden in zip(ras[ok][uk],decs[ok][uk],runs[ok][uk],nights[ok][uk],numbers[ok][uk],targets[ok][uk],ids[ok][uk]):
        print '<tr><td> %6.3f </td><td> %7.3f </td><td> %s </td><td> %s </td><td class="cen"> %s </td><td> %s </td><td> %s </td></tr>' % (ra,dec,run,night,number,target,iden)
    else:
