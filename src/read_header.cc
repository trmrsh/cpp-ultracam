...
/**
 * Interpret the ULTRACAM header info. This is the routine that handles all
 * the ULTRACAM timing stuff. It carries out byte swapping depending upon the
 * endian-ness of the machine.  \param buffer pointer to start of header
 * buffer \param serverdata data from the XML file needed for interpreting the
 * times. This will be slightly modified to reflect whether the data is
 * affected by the timestamping bug of Dec 2004 \param timing all the timing
 * info derived from the header (returned)
 */
...
void Ultracam::read_header(char* buffer, const Ultracam::ServerData& serverdata, Ultracam::TimingInfo& timing){
    ...
    if(serverdata.instrument == "ULTRASPEC" && serverdata.version == -1){
        ...
        format = 2;
             ...
             serverdata.version == 80127){
        format = 1;
    }else if(serverdata.version == 100222 || serverdata.version == 110921 ||
             serverdata.version == 111205 || serverdata.version == 120716 ||
             serverdata.version == 120813 || serverdata.version == 130307 ||
             serverdata.version == 130317 || serverdata.version == 140331){
        format = 2;
    }else{
    ...
    if(format == 1){
        ...
        if(reliable && (tstamp & PCPS_ANT_FAIL)){
            ...
            std::cerr << "Ultracam::read_header WARNING: time unreliable: "
                      << reason << std::endl;
            reliable = false;
        }
    ...
    bool bad_blue = (serverdata.nblue > 1) &&
        ((format == 1 && (buffer[0] & 1<<3)) ||
    ...
    // Clock board was changed in July 2003 and this resulted in the wrong
    // sense of bit change for the timestamps. Thus the timing code has to
    // change in between this date and the date when it was fixed in early
    // 2005. Basically the timestamps started to occur immediately after
    // readout as opposed to immediately prior to the frame shift into the
    // masked region.

    // This happened again when the GPS changed in March 2010. Extract from
    // e-mail from Dave Atkinson 11 June 2010:
    //
    // Apps3,4,5b
    // Loop:
    // CLEAR CCD
