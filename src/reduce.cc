	...
	for(int npass=1; npass<=maxpass; npass++){
		...
		if(npass == 1){
				...
				if((!blue_is_bad || nccd != 2) && Reduce::extraction_control.find(nccd) != Reduce::extraction_control.end()){
					...
					if(app.valid() && !app.linked()){
					    Ultracam::extract_flux(data[nccd], dvar[nccd], bad[nccd], Reduce::gain_frame[nccd], Reduce::bias_frame[nccd], 
								   ...
								   Reduce::NORMAL, zapped[nccd][naper], shape[nccd], Reduce::pepper[nccd], Reduce::saturation[nccd],
			...
			if((!blue_is_bad || nccd != 2) && Reduce::extraction_control.find(nccd) != Reduce::extraction_control.end()){
				...
				if(rsky1 >= rsky2)
				    throw Ultracam_Error("reduce: inner radius of sky annulus >= outer; should not happen. Probably need to change extraction_control parameters");
			    }
			...
			for(size_t nccd=0; nccd<data.size(); nccd++){ 
				...
				for(size_t naper=0; naper<aperture[nccd].size(); naper++){
					...
					if(Reduce::star_radius.size() > 0){
					    float rstar = 0;
					    if(Reduce::extraction_control[nccd].aperture_type       == Reduce::VARIABLE){
						rstar = Subs::clamp(Reduce::extraction_control[nccd].star_min, float(shape[nccd].fwhm*Reduce::star_radius[nradius]), Reduce::extraction_control[nccd].star_max);
					    }else if(Reduce::extraction_control[nccd].aperture_type == Reduce::FIXED){
						rstar = Subs::clamp(Reduce::extraction_control[nccd].star_min, Reduce::star_radius[nradius], Reduce::extraction_control[nccd].star_max);
					    }
					    app.set_rstar(rstar);
					}
