...
void Ultracam::parseXML(char source, const std::string& XML_URL, Ultracam::Mwindow& mwindow, Subs::Header& header, 
    ...
    const int NXTOT = serverdata.instrument == "ULTRACAM" ? 1080 : 1072; // largest ever X dimension
	...
	for(size_t nwin=0; nwin<uinfo.wind.size(); nwin++){
	    ...
	    if(nwin % 2 == 0){
		if(serverdata.instrument == "ULTRACAM" && serverdata.version < 0){
		    mwindow[nccd].push_back(Window(wind.llx+ncol*uinfo.xbin, wind.lly+nrow*uinfo.ybin,
						   wind.nx-ncol-1, wind.ny-nrow, uinfo.xbin, uinfo.ybin, NXTOT, NYTOT));
		}else{
		    if(reverse){
			mwindow[nccd].push_back(Window(NXTOT + 2 - wind.llx - wind.nx*uinfo.xbin, wind.lly+nrow*uinfo.ybin,
						       wind.nx-ncol, wind.ny-nrow, uinfo.xbin, uinfo.ybin, NXTOT, NYTOT));
		    }else{
			mwindow[nccd].push_back(Window(wind.llx+ncol*uinfo.xbin, wind.lly+nrow*uinfo.ybin,
						       wind.nx-ncol, wind.ny-nrow, uinfo.xbin, uinfo.ybin, NXTOT, NYTOT));
		    }
		...
		if(serverdata.instrument == "ULTRACAM" && serverdata.version < 0){
		    mwindow[nccd].push_back(Window(wind.llx+uinfo.xbin, wind.lly+nrow*uinfo.ybin, 
						   wind.nx-ncol-1, wind.ny-nrow, uinfo.xbin, uinfo.ybin, NXTOT, NYTOT));
		}else{
		    if(reverse){
			mwindow[nccd].push_back(Window(NXTOT + 2 - wind.llx - wind.nx*uinfo.xbin, wind.lly+nrow*uinfo.ybin, 
			...
			mwindow[nccd].push_back(Window(wind.llx, wind.lly+nrow*uinfo.ybin, 
						       wind.nx-ncol, wind.ny-nrow, uinfo.xbin, uinfo.ybin, NXTOT, NYTOT));
		    }
		}
	    }
	...
	throw Input_Error("parseXML error: framesize expected = " + Subs::str(framesize) + 
			  " but found = " + Subs::str(serverdata.framesize) );
